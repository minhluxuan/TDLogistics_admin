...
const distanceBaseDebugging = (info, moduleArray) => {
    ...
    for(let i = 0; i < info.length; i++) {
        ...
        if(checkDetailMass.errorVariale) return checkDetailMass;
    }
    return new Object({
        errorVariale: null,
        errorType: null,
...
const CPNdebugger = () => {
    ...
    if(innerProvinceChecking.errorVariale) return innerProvinceChecking;
    ...
    if(outerProvinceCheckingOrdinaryCase.errorVariale) return outerProvinceCheckingOrdinaryCase;
    ...
    if("special_case" in CPN.outer_province) {
        ...
        for(let i = 0; i < specialCase.length; i++) {
            ...
            if(detailMassCheck.errorVariale) return new Object({
                errorVariale: detailMassCheck.errorVariale,
