...
const find = async (pool, table, fields = null, values = null, desc = false, limit = null, offset = null) => {
    if (desc) {
        if (fields !== null && values !== null && fields.length > 0 && values.length > 0) {
            const whereClause =  fields.map(field => `${field} = ? `).join(' AND ');
            query = `SELECT * FROM ${table} WHERE ${whereClause} ORDER BY created_at DESC`;
