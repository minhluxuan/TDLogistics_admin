...
class StaffValidation {
    ...
    validateFindingStaffByStaff = (data) => {
        const schema = Joi.object({
            staff_id: Joi.string().pattern(new RegExp(process.env.REGEX_PERSONNEL)).required(),
        }).strict();
...
class AgencyValidation {
    ...
    validateCreatingAgency = (data) => {
        const schema = Joi.object({
            ...
            .noWhiteSpaces()
            .required(),
            user_fullname: Joi.string().regex(new RegExp(process.env.REGEX_NAME)),
            user_phone_number: Joi.string().regex(new RegExp(process.env.REGEX_PHONE_NUMBER)),
            user_email: Joi.string().pattern(new RegExp(process.env.REGEX_EMAIL)),
            user_date_of_birth: Joi.string().regex(new RegExp(process.env.REGEX_BIRTHDAY)),
            user_cccd: Joi.string().regex(new RegExp(process.env.REGEX_CCCD)),
    ...
    isAllowedToCreate = (creatorId, agency) => {
        const levels = ["AU", "SU", "AG", "SG", "AC", "SC", "AP", "SP", "AD", "SD", "AT", "ST"];
        ...
        if ((creatorLevel === 6 || creatorLevel === 7) && creatorIdArray[2].slice(0, 2) !== agency.postal_code.slice(0, 2)
        || (creatorLevel === 8|| creatorLevel === 9) && creatorIdArray[2].slice(0, 4) !== agency.postal_code.slice(0, 4)
        || (creatorLevel === 10|| creatorLevel === 11) && creatorIdArray[2].slice(0, 5) !== agency.postal_code.slice(0, 5)) {
            logger.info(`User ${creatorId} is not permitted to create an agency with level: ${agency.level}, postal code: ${agency.postal_code}`);
            return {
                allowed: false,
    ...
    isAllowedToDelete = (deletorId, agency) => {
        const levels = ["AU", "SU", "AG", "SG", "AC", "SC", "AP", "SP", "AD", "SD", "AT", "ST"];
        const deletorIdArray = deletorId.split('_');
        const deletorLevel = levels.indexOf(deletorIdArray[1]);

        if (deletorIdArray[0] !== "TD") {
            logger.info(`User ${deletorId} is not permitted to create an agency with level: ${agency.level}, postal code: ${agency.postal_code}`);
            return {
                allowed: false,
                message: "Hành động không được cho phép. Người dùng không được phép truy cập tài nguyên này.",
            }
        ...
        if ((deletorLevel === 6 || deletorLevel === 7) && deletorIdArray[2].slice(0, 2) !== agency.postal_code.slice(0, 2)
        || (deletorLevel === 8|| deletorLevel === 9) && deletorIdArray[2].slice(0, 4) !== agency.postal_code.slice(0, 4)
        || (deletorLevel === 10|| deletorLevel === 11) && deletorIdArray[2].slice(0, 5) !== agency.postal_code.slice(0, 5)) {
            logger.info(`User ${deletorId} is not permitted to create an agency with level: ${agency.level}, postal code: ${agency.postal_code}`);
            return {
                allowed: false,
                message: "Hành động không được cho phép. Địa phương của đại lý không thuộc quyền kiểm soát của người dùng.",
        ...
        logger.info(`User ${deletorId} is permitted to create an agency with level: ${agency.level}, postal code: ${agency.postal_code}`);
        return {
            allowed: true,
            message: "Hành động được cho phép.",
        }
