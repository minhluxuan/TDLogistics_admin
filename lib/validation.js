...
class ShipmentValidation {
    ...
    validateUndertakeShipment = (data) => {
        const schema = Joi.object({
            shipment_id: Joi.string().alphanum().required(),
            status_code: Joi.number().valid(1, 2),
        }).strict();
        return schema.validate(data);
    }
...
class OrderValidation {
    validateFindingOrders = (data) => {
        const schema = Joi.object({
            name_reciever: Joi.string().pattern(new RegExp(process.env.REGEX_NAME)),
            phone_reciever: Joi.string().pattern(new RegExp(process.env.REGEX_PHONE_NUMBER)),
