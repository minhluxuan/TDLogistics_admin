  ...
  validateCreatingStaffByAgency = (data) => {
    const schema = Joi.object({
      ...
      phone_number: Joi.string()
        .regex(new RegExp(process.env.REGEX_PHONE_NUMBER))
        .required(),
      role: Joi.string()
        .valid(
          "AGENCY_MANAGER",
  ...
  validateCreatingStaffByAdmin = (data) => {
    const schema = Joi.object({
      agency_id: Joi.string()
        .regex(new RegExp(process.env.REGEX_PERSONNEL))
        .required(),
      fullname: Joi.string()
        .regex(new RegExp(process.env.REGEX_NAME))
        .required(),
      username: Joi.string().required(),
      password: joiPassword
        .string()
  ...
  validateFindingStaffByAdmin = (data) => {
    const schema = Joi.object({
      ...
      phone_number: Joi.string().regex(
        new RegExp(process.env.REGEX_PHONE_NUMBER)
      ),
...
class ShipmentValidation {
  ...
  validateCreatingShipment = (data) => {
    const schema = Joi.object({
      transport_partner_id: Joi.string().regex(
        new RegExp(process.env.REGEX_TRANSPORT_PARTNER)
      ),
    }).strict();
  ...
  validateQueryUpdatingShipment = (data) => {
    const schema = Joi.object({
      shipment_id: Joi.string()
        .regex(new RegExp(process.env.REGEX_SHIPMENT_ID))
        .required(),
    }).strict();
  ...
  validateFindingShipment = (data) => {
    const schema = Joi.object({
      shipment_id: Joi.string().regex(
        new RegExp(process.env.REGEX_SHIPMENT_ID)
      ),
      transport_partner_id: Joi.string().regex(
        new RegExp(process.env.REGEX_TRANSPORT_PARTNER)
      ),
  ...
  validateDecomposingShipment = (data) => {
    const schema = Joi.object({
      order_ids: Joi.array()
        .items(Joi.string().regex(new RegExp(process.env.REGEX_ORDER_ID)))
        .required(),
    }).strict();
  ...
  validateUndertakeShipment = (data) => {
    const schema = Joi.object({
      shipment_id: Joi.string()
        .regex(new RegExp(process.env.REGEX_SHIPMENT_ID))
        .required(),
      status_code: Joi.number().valid(5, 7),
    }).strict();
    return schema.validate(data);
  };
}
  ...
  validateCreateBusinessByAgency = (data) => {
    const schema = Joi.object({
      ...
      user_email: Joi.string()
        .regex(new RegExp(process.env.REGEX_EMAIL))
        .required(),
      user_date_of_birth: Joi.string()
        .regex(new RegExp(process.env.REGEX_BIRTHDAY))
        .required(),
      user_cccd: Joi.string()
  ...
  validateCheckingExistBusiness = (data) => {
    const schema = Joi.object({
      tax_number: Joi.string()
        .regex(new RegExp(process.env.REGEX_TAX_CODE))
        .required(),
    }).strict();
  ...
  validateQueryUpdatingBusiness = (data) => {
    const schema = Joi.object({
      business_id: Joi.string()
        .regex(new RegExp(process.env.REGEX_BUSINESS))
        .required(),
    }).strict();
  ...
  validateDeletingBusiness = (data) => {
    const schema = Joi.object({
      business_id: Joi.string()
        ...
        .required(),
      agency_id: Joi.string()
        .regex(new RegExp(process.env.REGEX_AGENCY_ID))
        .required(),
    }).strict();
...
class VehicleValidation {
  ...
  validateCheckingExistVehicle = (data) => {
    const schema = Joi.object({
      vehicle_id: Joi.string()
        .regex(new RegExp(process.env.REGEX_VEHICLE))
        .required(),
    });
  ...
  validateCreatingVehicleByAgency = (data) => {
    const schema = Joi.object({
      ...
      license_plate: Joi.string()
        .regex(new RegExp(process.env.REGEX_LICENSE_PLATE))
        .required(),
  ...
  validateUpdatingVehicle = (data) => {
    const schema = Joi.object({
      ...
      staff_id: Joi.string().regex(new RegExp(process.env.REGEX_PERSONNEL)),
      type: Joi.string(),
      license_plate: Joi.string().regex(
        new RegExp(process.env.REGEX_LICENSE_PLATE)
      ),
  ...
  validateShipmentIds = (data) => {
    const schema = Joi.object({
      shipment_ids: Joi.array().items(
        Joi.string().regex(new RegExp(process.env.REGEX_SHIPMENT_ID))
      ),
    }).strict();
...
class AuthorizationValidation {
  ...
  isAllowedToGrant = (
    ...
    if (
      orders.indexOf(granterArray[0]) > orders.indexOf(receiverArray[0]) ||
      (orders.indexOf(granterArray[0]) > 0 &&
        granterArray[1] !== receiverArray[1]) ||
      (orders.indexOf(granterArray[0]) > 1 &&
        granterArray[2] !== receiverArray[2]) ||
      (orders.indexOf(granterArray[0]) > 2 &&
        granterArray[3] !== receiverArray[3])
    ) {
  ...
  isAllowedToRevoke = (
    ...
    if (
      orders.indexOf(revokerArray[0]) > orders.indexOf(loserArray[0]) ||
      (orders.indexOf(revokerArray[0]) > 1 &&
        revokerArray[1] !== loserArray[1]) ||
      (orders.indexOf(revokerArray[0]) > 3 &&
        revokerArray[2] !== loserArray[2]) ||
  ...
  isAllowedToRead = (reader_id, read_object_id, primaryPermissionOfReader) => {
    ...
    if (
      orders.indexOf(readerArray[0]) > orders.indexOf(readObjectArray[0]) ||
      (orders.indexOf(readerArray[0]) > 1 &&
        readerArray[1] !== readObjectArray[1]) ||
      (orders.indexOf(readObjectArray[0]) > 3 &&
        readerArray[2] !== readObjectArray[2]) ||
      (orders.indexOf(readObjectArray[0]) > 5 &&
        readerArray[3] !== readObjectArray[3])
    ) {
      console.log("Action is not allowed.");
      return false;
    }
...
class PartnerStaffValidation {
  ...
  validateCreatingPartnerStaff = (data) => {
    const schema = Joi.object({
      partner_id: Joi.string()
        .regex(new RegExp(process.env.REGEX_TRANSPORT_PARTNER))
        .required(),
      username: Joi.string().required(),
      password: joiPassword
        .string()
  ...
  validateGetPartnerAvatarAndLicense = (data) => {
    const schema = Joi.object({
      staff_id: Joi.string().regex(new RegExp(process.env.REGEX_PERSONNEL)),
    }).strict();
  ...
  validateCreatingPartnerByAdmin = (data) => {
    const schema = Joi.object({
      ...
      user_fullname: Joi.string()
        .regex(new RegExp(process.env.REGEX_NAME))
        .required(),
      user_phone_number: Joi.string().regex(
        new RegExp(process.env.REGEX_PHONE_NUMBER)
      ),
      user_email: Joi.string().regex(new RegExp(process.env.REGEX_EMAIL)),
      user_date_of_birth: Joi.string().regex(
        new RegExp(process.env.REGEX_BIRTHDAY)
      ),
      user_cccd: Joi.string()
        .regex(new RegExp(process.env.REGEX_CCCD))
        .required(),
...
class AgencyValidation {
  ...
  validateCreatingAgency = (data) => {
    const schema = Joi.object({
      ...
      postal_code: Joi.string()
        .regex(new RegExp(process.env.REGEX_POSTAL_CODE))
        .required(),
      ...
      bin: Joi.string().regex(new RegExp(process.env.REGEX_BIN)),
      bank: Joi.string(),
      invidual_company: Joi.number().required(),

      // info if not invidual company
  ...
  validateFindingLicenseAgencyCompany = (data) => {
    const schema = Joi.object({
      agency_id: Joi.string()
        .regex(new RegExp(process.env.REGEX_AGENCY_ID))
        .required(),
...
class OrderValidation {
  ...
  validateFindingOrders = (data) => {
    const schema = Joi.object({
      ...
      phone_receiver: Joi.string().pattern(
        new RegExp(process.env.REGEX_PHONE_NUMBER)
      ),
  ...
  validateCreatingOrder = (data) => {
    const schema = Joi.object({
      name_receiver: Joi.string()
        .pattern(new RegExp(process.env.REGEX_NAME))
        .required(),
      phone_number_receiver: Joi.string()
        .pattern(new RegExp(process.env.REGEX_PHONE_NUMBER))
        .required(),
  ...
  validateCreatingOrderByAdmin = (data) => {
    const schema = Joi.object({
      ...
      name_sender: Joi.string()
        .pattern(new RegExp(process.env.REGEX_NAME))
        .required(),
      phone_number_sender: Joi.string()
        .pattern(new RegExp(process.env.REGEX_PHONE_NUMBER))
        .required(),
      name_receiver: Joi.string()
        .pattern(new RegExp(process.env.REGEX_NAME))
  ...
  validateQueryUpdatingOrder = (data) => {
    const schema = Joi.object({
      order_id: Joi.string()
        .regex(new RegExp(process.env.REGEX_ORDER_ID))
        .required(),
    }).strict();
