...
class OrderValidation {
    ...
    validateCreatingOrder = (data) => {
        const schema = Joi.object({            
            ...
            long_dest: Joi.number().min(-180).max(180).required(),
            lat_dest: Joi.number().min(-90).max(90).required(),
    ...
    validateCreatingOrderByAdmin = (data) => {
        const schema = Joi.object({
            ...
            long_dest: Joi.number().min(-180).max(180),
            lat_dest: Joi.number().min(-90).max(90),
            COD: Joi.number().min(0),
            service_type: Joi.number().min(0).required()
        }).strict();
