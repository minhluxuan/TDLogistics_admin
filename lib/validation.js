...
class OrderValidation {
    ...
    validateCreatingOrder = (data) => {
        const schema = Joi.object({
            ...
            phone_receiver: Joi.string().pattern(new RegExp(process.env.REGEX_PHONE_NUMBER)).required(),
            mass: Joi.number().precision(2).min(0).required(),
    ...
    validateCreatingOrderByAgency = (data) => {
        const schema = Joi.object({
...
class EmailValidation {
    validateCreateEmail = (data) => {
        const schema = Joi.object({
            email: Joi.array().items(string().email()).required(),
            subject: Joi.string().required(),
            content: Joi.string.required(),
        }).strict();
        return schema.validate(data);
    };
    validateDeleteEmail = (data) => {
        const schema = Joi.object({
            id: Joi.string().required(),
        }).strict();
        return schema.validate(data);
    };
    validateFindingEmail = (data) => {
        const schema = Joi.object({
