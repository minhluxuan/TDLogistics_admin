...
class StaffValidation {
    ...
    validateFindingStaffByAdmin = (data) => {
        const schema = Joi.object({
            ...
            fullname: Joi.regex(new RegExp(process.env.REGEX_NAME)).string(),
            username: Joi.string(),
...
class ShipmentValidation {
    ...
    validateUndertakeShipment = (data) => {
        const schema = Joi.object({
            shipment_id: Joi.string().regex(new RegExp(process.env.REGEX_SHIPMENT_ID)).required(),
            status_code: Joi.number().valid(3, 6)
        }).strict();
        return schema.validate(data);
    }
...
class VehicleValidation {
    ...
    validateOrderIds = (data) => {
        const schema = Joi.object({
            order_ids: Joi.array().items(Joi.string().regex(new RegExp(process.env.REGEX_ORDER_ID))),
        }).strict();
...
class OrderValidation {
    ...
    validateCreatingOrder = (data) => {
        const schema = Joi.object({
            ...
            service_type: Joi.number().valid(1, 2, 3, 4 ,5).required()
        }).strict();
