...
class PaymentValidation {
    validatePaymentResult = (data) => {
        const schema = Joi.object({
            code: Joi.alphanumeric().length(2),
            desc: Joi.string(),
            data: {
            id: Joi.alphanumeric(),
            orderCode: Joi.string().regex(new RegExp(process.env.REGEX_ORDER_ID)),
            amount: Joi.number().min(0),
            amountPaid: Joi.number().min(0),
            amountRemaining: Joi.number().min(0),
            status: Joi.string().required(),
            createdAt: Joi.date(),
            transactions: Joi.array(),
            cancellationReason: Joi.string().required(),
            canceledAt: Joi.string().required(),
            },
            signature: Joi.alphanumeric()
        });
