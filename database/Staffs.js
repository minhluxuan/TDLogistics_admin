...
const getManyStaffs = async (info, postal_code) => {
    ...
    if (!postal_code) {
        if (fields.length === 0 || values.length === 0) {
            ...
            query = `SELECT agency_id, username, phone_number, email, fullname, date_of_birth, cccd, province, district, town, detail_address, staff_id, position, bin, bank, deposit, salary, paid_salary, active FROM staff WHERE ${fields
                .map((field) => `${field} = ?`)
                .join(" AND ")}`;
        }
    } else {
        if (fields.length === 0 || values.length === 0) {
            query = `SELECT agency_id, username, phone_number, email, fullname, date_of_birth, cccd, province, district, town, detail_address, staff_id, position, bin, bank, deposit, salary, paid_salary, active FROM ${
                postal_code + "_" + "staff"
            }`;
        } else {
            query = `SELECT agency_id, username, phone_number, email, fullname, date_of_birth, cccd, province, district, town, detail_address, staff_id, position, bin, bank, deposit, salary, paid_salary, active FROM ${
...
const updateStaff = async (info, conditions, postalCode = null) => {
    ...
    if (postalCode) {
        return await dbUtils.updateOne(
            pool,
            postalCode + "_" + table,
            fields,
            values,
