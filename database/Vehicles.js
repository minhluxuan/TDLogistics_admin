...
const getManyVehicles = async (conditions, paginationConditions) => {
    ...
    if (fields && values && fields.length > 0 && values.length > 0) {
        query = `SELECT v.transport_partner_id, v.agency_id, v.staff_id, v.vehicle_id, v.type, v.license_plate, 
                ...
                WHERE v.transport_partner_id IS NOT NULL AND v.transport_partner_id != "" AND ${fields.map(field => `v.${field} = ?`).join(" AND ")}
                ...
                WHERE v.transport_partner_id IS NULL OR v.transport_partner_id = "" AND ${fields.map(field => `v.${field} = ?`).join(" AND ")}
