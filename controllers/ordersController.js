            ...
            socket.on("notifyNewOrderFromUser", (info) => {
                try {
                    ...
                    if (["USER"].includes(socket.request.user.role)) {
                        ...
                        ["MANAGER", "TELLER", "AGENCY_MANAGER", "AGENCY_TELLER"].includes(socket.request.user.role)
                    ) {
...
const createNewOrder = async (socket, info, orderTime) => {
    try {
        const resultFindingManagedAgency = await ordersService.findingManagedAgency(
            info.ward_source,
            info.district_source,
            info.province_source
        );
        ...
        info.order_id =
            areaAgencyIdSubParts[0] +
            "_" +
            areaAgencyIdSubParts[1] +
            "_" +
            orderTime.getFullYear().toString() +
            (orderTime.getMonth() + 1).toString() +
            orderTime.getDate().toString() +
            orderTime.getHours().toString() +
            orderTime.getMinutes().toString() +
            orderTime.getSeconds().toString() +
            orderTime.getMilliseconds().toString();
        const addressSource =
            info.detail_source + ", " + info.ward_source + ", " + info.district_source + ", " + info.province_source;
        const addressDest =
            info.detail_dest + ", " + info.ward_dest + ", " + info.district_dest + ", " + info.province_dest;
        ...
        const resultCreatingNewOrderInAgency = await ordersService.createOrderInAgencyTable(
            info,
            resultFindingManagedAgency.postal_code
        );
...
const checkExistOrder = async (req, res) => {
    try {
        ...
        return res.status(200).json({
            ...
            existed: existed,
            message: existed
                ? `Đơn hàng có mã ${req.query.order_id} đã tồn tại.`
                : `Đơn hàng có mã ${req.query.order_id} không tồn tại.`,
        });
    } catch (error) {
...
const updateOrder = async (req, res) => {
    try {
        const { error } =
            OrderValidation.validateQueryUpdatingOrder(req.query) || OrderValidation.validateUpdatingOrder(req.body);
