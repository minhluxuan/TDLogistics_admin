...
const checkExistAgency = async (req, res) => {
    try {
        ...
        return res.status(200).json({
            ...
            existed: existed,
            message: existed
                ? `Bưu cục có mã bưu cục ${req.query.agency_id} đã tồn tại.`
                : `Bưu cục có mã bưu cục ${req.query.agency_id} chưa tồn tại.`,
        });
    } catch (error) {
        res.status(500).json({
            error: true,
...
const getAgencies = async (req, res) => {
    try {
        ...
        if (
            ["ADMIN", "MANAGER", "HUMAN_RESOURCE_MANAGER", "TELLER", "COMPLAINTS_SOLVER"].includes(req.user.role) ||
            req.user.privileges.includes(2)
        ) {
